...
def lambda_handler(event, context):
    ...
    if event["resource"] == "/datasets":
        ...
        return route_datasets_id(request_params, event["pathParameters"].get("id", None))

    elif event['resource'] == '/datasets/{id}/g_variants':
        return route_datasets_id_g_variants(request_params, event["pathParameters"].get("id", None))

    elif event['resource'] == '/datasets/{id}/biosamples':
        return route_datasets_id_biosamples(request_params, event["pathParameters"].get("id", None))

    elif event['resource'] == '/datasets/{id}/individuals':
        return route_datasets_id_individuals(request_params, event["pathParameters"].get("id", None))

    elif event['resource'] == '/datasets/{id}/filtering_terms':
        return route_datasets_id_filtering_terms(request_params, event["pathParameters"].get("id", None))

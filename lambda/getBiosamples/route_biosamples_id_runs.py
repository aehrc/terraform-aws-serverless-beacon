...
def route(event):
    ...
    if len(filters) > 0:
        ...
        runs_filters = ','.join(map(lambda y: f"'{y['id']}'", filter(lambda x: x.get('scope', 'runs') == 'runs', filters)))
        conditions = f''' id IN (SELECT id FROM {TERMS_INDEX_TABLE} WHERE kind='runs' AND term IN ({runs_filters})) '''

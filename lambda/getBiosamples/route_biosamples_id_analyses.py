...
def route(event):
    ...
    if len(filters) > 0:
        ...
        analysis_filters = ','.join(map(lambda y: f"'{y['id']}'", filter(lambda x: x.get('scope', 'analyses') == 'analyses', filters)))
        conditions = f''' id IN (SELECT id FROM {TERMS_INDEX_TABLE} WHERE kind='analyses' AND term IN ({analysis_filters})) '''

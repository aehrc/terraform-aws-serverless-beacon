    ...
    for f in filters:
        ...
        if len(f_id)==1 and f_id[0] in id_class._table_columns:
            ...
            if len(f_id)==2 and f_id[0] in queried_athena_models.keys() and f_id[1] in queried_athena_models[f_id[0]]._table_columns:
                ...
                join_constraints.append(f''' SELECT RI.{type_relations_table_id[id_type]} FROM "{RELATIONS_TABLE}" RI JOIN "{joined_class._table_name}" TI ON RI.{type_relations_table_id[group]} = TI.id where and TI.{comparrison} ''')
            # if none of these two cases match, assume it is an ontology term
                ...
                join_constraints.append(f''' SELECT RI.{type_relations_table_id[id_type]} FROM "{RELATIONS_TABLE}" RI JOIN "{TERMS_INDEX_TABLE}" TI ON RI.{type_relations_table_id[group]} = TI.id where and TI.term IN ({expanded_terms}) ''')
